...
public String userLogin(@ModelAttribute("userLogin") UserLogin login, Model model) {
    ...
    // Log pour vérifier si l'email et le mot de passe sont bien récupérés
    System.out.println("Email: " + email + ", Password: " + password);


	if ("admin@gmail.com".equals(email) && "admin123".equals(password)) {
        // Rediriger vers la page d'administration si l'email et le mot de passe correspondent
       return "redirect:/admin/services"; // Assurez-vous que cette page existe
	   //return "Admin_Page";
    }
    ...
    if (user != null && services.validateLoginCredentials(email, password)) {
        List<Orders> orders = this.orderServices.getOrdersForUser(user);
        model.addAttribute("orders", orders);
        model.addAttribute("name", user.getUname());
	...
	public String registerUser(@ModelAttribute("user") User user, 
		...
		if (!user.getUpassword().equals(confirmPassword)) {
			model.addAttribute("error", "Les mots de passe ne correspondent pas.");
			return "Register"; // Retourner à la page d'inscription avec un message d'erreur
		}
		
		// Vérification si l'email existe déjà
		if (services.getUserByEmail(user.getUemail()) != null) {
			model.addAttribute("error", "Cet email est déjà utilisé.");
			return "Register"; // Retourner à la page d'inscription si l'email existe déjà
		}
	
		// Aucun hachage ici, le mot de passe est stocké tel quel
		services.addUser(user);  // L'utilisateur avec le mot de passe en clair est enregistré
		...
		return "Login"; // Vous pouvez rediriger vers la page de connexion après l'inscription
	}
